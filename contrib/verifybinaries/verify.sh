...
###   otherbcore.org and otherb.org and compares them.
...
WORKINGDIR="/tmp/otherb_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://otherbcore.org"
HOST2="https://otherb.org"
BASEDIR="/bin/"
VERSIONPREFIX="otherb-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'otherb-',
...
if [ $? -ne 0 ]; then
   echo "otherb.org failed to provide signature file, but otherbcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "otherb.org and otherbcore.org signature files were not equal?"
