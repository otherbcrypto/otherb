...
*Setup instructions for a Gitian build of otherb Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the otherb
...
- [Building otherb Core](#building-otherb-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-otherb-host/README.md).
...
Clone the git repositories for otherb and Gitian.
...
in the otherb repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building otherb Core
----------------

To build otherb Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the otherb repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/otherb/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/otherb-project/otherb
...
For example:
```bash
URL=https://github.com/thrasher-/otherb.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit otherb=${COMMIT} --url otherb=${URL} ../otherb/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit otherb=${COMMIT} --url otherb=${URL} ../otherb/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit otherb=${COMMIT} --url otherb=${URL} ../otherb/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the otherb git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../otherb/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/otherb
SIGPATH=/some/root/path/otherb-detached-sigs

./bin/gbuild --url otherb=${BTCPATH},signature=${SIGPATH} ../otherb/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/otherb-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/otherb-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/otherb-osx-build.assert
```
...
[otherb-project/gitian.sigs.otb](https://github.com/otherb-project/gitian.sigs.otb/) repository, or if that's not possible create a pull
