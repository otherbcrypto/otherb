otherb Core version 0.13.2 is now available from:

  <https://download.otherb.org/otherb-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a otherb
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Bitcoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/otherb/otherb/issues/7681#issuecomment-217439891)
...
ECDSA signatures inside otherb transactions now use validation using
[libsecp256k1](https://github.com/otherb-core/secp256k1) instead of OpenSSL.
...
(https://github.com/otherb/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of otherb Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of otherb Core was to raise the default minimum
relay fee.

otherb Core 0.13.2 will have a strict maximum size on the mempool. The
...
otherb Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running otherbd without having to do any manual configuration.
...
otherb Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. otherb Core will relay transactions with
...
entirely in otherb Core 0.13, and it is at this time undecided whether the
...
otherb Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
otherb Core automatically creates a hidden service to listen on, without
manual configuration. otherb Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if otherb Core is listening, and
...
otherbd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals otherb
Core to use floating fees. By default, otherb Core will use floating
fees.
...
(default: `0.0002` OTB/kB).

At all times, otherb Core will cap fees at `-maxtxfee=<x>` (default:
0.10) OTB.
Furthermore, otherb Core will never create transactions paying less than
...
`otherb.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of Bitcoin Core v0.11.0](https://github.com/otherb/otherb/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/otherb/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/otherb/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /otherbrpc>
        ...
        # optional bypass otherbd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< otherbrpc:<password>
    </Location>
...
Restarting otherbd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in otherb.conf
...
For example:
    ...
    $ src/otherb-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The otherb Core code base has
...
- `otherb-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `otherb-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
<https://github.com/otherb/otherb/pull/7184>

BIP112 soft fork to enforce OP_CHECKSEQUENCEVERIFY
--------------------------------------------------

[BIP112][] redefines the existing OP_NOP3 as OP_CHECKSEQUENCEVERIFY (CSV)
for a new opcode in the otherb scripting system that in combination with
[BIP68][] allows execution pathways of a script to be restricted based
on the age of the output being spent.

For more information about the implementation, see
<https://github.com/otherb/otherb/pull/7524>
...
otherb transactions currently may specify a locktime indicating when
...
For more information about the implementation, see
<https://github.com/otherb/otherb/pull/6566>
...
compatible peers.
[BIP 152](https://github.com/otherb/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of otherb Core.

[Pull request](https://github.com/otherb/otherb/pull/8035/files), [BIP 32](https://github.com/otherb/bips/blob/master/bip-0032.mediawiki)
...
Witness)] (https://github.com/otherb/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/otherb/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the otherb-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/otherb/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/otherb/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
112](https://github.com/otherb/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/otherb/otherb/pull/7762).
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming otherb wallet
  software more difficult and which seriously complicates the design of smart
  contracts for otherb.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each otherb block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of otherbs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of otherbs being spent is exactly
...
- **Increased security for multisig:** otherb addresses (both P2PKH addresses
  ...
  believed to be provided by otherb's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of otherb's current ledger (called the UTXO set),
  ...
  note that otherb Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  otherb users to individually opt-in to almost any change in the otherb
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and otherb Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the otherb
client to otherb Core 0.13.2 to signal for segwit support. When a
...
[Segwit FAQ]: https://otherbcore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://otherbcore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/otherb/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/otherb/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/otherb/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/otherb/bips/blob/master/bip-0145.mediawiki
...
Since otherb Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/otherb/bips/blob/master/bip-0147.mediawiki
...
- `otherb-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `otherb-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
