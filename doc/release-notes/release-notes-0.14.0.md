Bitcoin Core version 0.14.0 is now available from:
  ...
  <https://otherb.org/bin/otherb-core-0.14.0/>
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/otherb/otherb/blob/master/doc/zmq.md) for
...
The Alert System was [disabled and deprecated](https://otherb.org/en/alert/2016-11-01-alert-retirement) in Bitcoin Core 0.12.1 and removed in 0.13.0. 
...
- #8722 `bb843ad` otherb-cli: More detailed error reporting (laanwj)
...
- #8830 `ef0801b` test: Add option to run otherb-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to otherb-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to otherb-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and otherb-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_otherb_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change otherb address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` otherbd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow otherb-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` otherb-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to otherb-tx (jnewbery)
